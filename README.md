...
Due to recent news, you or your company might consider moving away from React.js or any technology under Facebook’s BSD+Patents License.

# React to Preact
There are three different approaches to switch from React to Preact:

## First Approach: Easiest
...
The most noteworthy of these is `createClass()`. Opinions vary wildly on the subject of classes and OOP. It is worth understanding that JavaScript classes are internally in VDOM libraries to represent component types. These component types are important when dealing with the nuances of managing component lifecycles.
...
Preact does not use a separate module for DOM rendering since it is focused solely on being an excellent DOM renderer.
...
Preact does not assume it controls the entire root element you pass it which makes this possible.  The second argument to `render()`
...
The `render()` returns the root element: you pass it as the third argument to re-render in-place.
